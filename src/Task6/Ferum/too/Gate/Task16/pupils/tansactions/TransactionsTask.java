...
public class TransactionsTask {
    public static void main(String[] args) {
       ...
       Map<String, Long> first = transactionStream // так как в мапе одинаковых ключей не будет
                     .collect(Collectors.toMap(
                                       transaction -> transaction.getAccount().getNumber(),
                                       suman -> suman.getSum(),
         (item1, item2) -> item1 + item2  // если нет совпадения ключа то данная строчка пропускается
        ));
